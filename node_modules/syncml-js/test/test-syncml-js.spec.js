...
define([
  ...
  describe('syncml-js', function() {
    ...
    //-------------------------------------------------------------------------
    it('does not recurse excessively', function(done) {
      ...
      async.series([
        ...
        // add many items
        function(cb) {
          async.mapSeries(_.range(count), function(item, cb) {
            sync.c1.storage.add({body: 'recurse-item-' + item}, function(err, item) {
              expect(err).ok();
              sync.c1.store.registerChange(item.id, syncml.ITEM_ADDED, null, function(err) {
                expect(err).ok();
                cb(err);
